...
<div class="w-full h-auto from-[#0f172a] to-[#1e293b] flex flex-col justify-start items-center p-4">
        ...
        <div class="w-full bg-[#1e293b] rounded-xl overflow-hidden">
            <div class="sticky top-0 h-12 bg-[#334155] flex items-center px-4">
                <div class="flex justify-between w-full text-sm text-[#e2e8f0] font-medium">
                    <span class="hidden sm:block w-1/4 text-left">Name</span>
                    <span class="block sm:hidden w-1/4 text-left">Information</span>
                    <span class="hidden sm:block w-1/4 text-left">Email</span>
                    <span class="hidden sm:block w-1/4 text-left">Phone</span>
            ...
            <div class="max-h-[calc(80vh-200px)] overflow-y-auto">
                ...
                <div class="min-h-[48px] py-2 border-b border-[#475569] flex items-center px-4 hover:bg-[#252f3f]">
                    <div class="flex flex-col sm:flex-row justify-between w-full text-sm text-[#e2e8f0] gap-1">
                        <span class="w-full sm:w-1/4 text-left">
                        ...
                        <span class="w-full sm:w-auto text-right sm:text-left">
                            <button 
    ...
    @isset($userIdToEdit)
        <x-modal name="confirm-user-edit" :show="isset($userIdToEdit)" focusable x-on:modal-closed="$wire.set('userIdToEdit', null)">
            <livewire:profile.update-profile-information-form :userToEdit="$userIdToEdit" />
        </x-modal>
    @endisset

    @isset($userIdToDelete)
        <x-modal name="confirm-user-delete" :show="isset($userIdToDelete)" focusable x-on:modal-closed="$wire.set('userIdToDelete', null)">
            <livewire:profile.delete-user-form :userToDelete="$userIdToDelete" />
        </x-modal>
    @endisset

    <x-modal name="confirm-user-create" :show="$errors->isNotEmpty()" focusable>
        <livewire:profile.create-profile-form />
    </x-modal>
</div>