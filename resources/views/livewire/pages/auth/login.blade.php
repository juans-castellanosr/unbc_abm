        ...
        <form wire:submit="login">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                @if (Route::has('password.request'))
                    <a class="text-sm text-blue-400 hover:text-blue-300 transition-colors duration-200" 
                        href="{{ route('password.request') }}" wire:navigate>
                        {{ __('Forgot your password?') }}
                    </a>
                @endif

                <x-primary-button class="w-full sm:w-auto bg-blue-500 hover:bg-blue-400 text-white px-6 py-3 rounded-xl transition-colors duration-200">
                    {{ __('Log in') }}
                </x-primary-button>
            </div>
        </form>
